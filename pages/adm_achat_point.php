...
<div class="leftside">
    <ol class="breadcrumb">
        ...
        <li class="active">Panel des achats</li>
    </ol>
    <div class="row">
        <div class="col-md-12 col-xs-12">
            <section class="no-border no-padding-top">
                <div class="page-header margin-top-10"><h4>Gestions des achats de points</h4></div>
            </section>
        </div>
    </div>
            ...
            <section class="section section-white no-border no-padding-top">
                <div class="section section-default padding-25">
                    ...
                    if(isset($_GET['wAchat']) && is_numeric($_GET['wAchat']) && isset($_GET['remove'])) {
                        $query = $web -> prepare("DELETE FROM `website_shop_points_purchases` WHERE `id` = ?;");
                        $query -> bindParam(1, $_GET['wAchat']);
                        $query -> execute();
                        $query -> closeCursor();
                        echo '<div class="alert alert-success no-border-radius no-margin" role="alert"><strong>Oh good!</strong> L\'achat a été supprimer avec succès !</div><br>';
                        echo '<script>setTimeout(function() {window.location.href = "?page=adm_achat_point";}, 1000); // 1000 millisecondes = 1 secondes</script>';
                    } ?>
                    <div class="box no-border-radius">
                        <table class="table table-striped no-margin">
                            ...
                            $queryCount = $web->prepare("SELECT COUNT(*) FROM `website_shop_points_purchases`;");
                            $queryCount->execute();
                            ...
                            $query = $web -> prepare("SELECT * FROM `website_shop_points_purchases` ORDER BY `id` ASC LIMIT $indexDebut, $achatParPage;");
                            $query -> execute();
                                ...
                                <tr>
                                    <td colspan="8">
                                        <div class='alert alert-info no-border-radius no-margin' style='text-align: center!important;' role='alert'>
                                            <strong>Oh non!</strong> Il n'y a aucun achat de points!
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
                    ...
                    <center>
                        <div class="btn-group">
                            <?php if ($page > 1) : ?>
                                <a href="?page=adm_achat_point&num=1&achatParPage=<?php echo $achatParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-angle-double-left"></i></a>
                                <a href="?page=adm_achat_point&num=<?php echo ($page - 1); ?>&ticketParPage=<?php echo $achatParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-chevron-left"></i></a>
                            <?php else : ?>
                            ...
                            echo '<a href="?page=adm_achat_point&num=1&achatParPage=' . $achatParPage . '" class="btn btn-sm ' . ($page == 1 ? 'btn-primary' : 'btn-default') . '">1</a>';
                            ...
                            for ($i = max(2, $page - 3); $i <= min($page + 3, $moyenne - 1); $i++) {
                                echo '<a href="?page=adm_achat_point&num=' . $i . '&achatParPage=' . $achatParPage . '" class="btn btn-sm ' . ($i == $page ? 'btn-primary' : 'btn-default') . '">' . $i . '</a>';
                            }
                            ...
                            echo '<a href="?page=adm_achat_point&num=' . $moyenne . '&achatParPage=' . $achatParPage . '" class="btn btn-sm ' . ($page == $moyenne ? 'btn-primary' : 'btn-default') . '">' . $moyenne . '</a>';
                            ?>
                            <?php if ($page < $moyenne) : ?>
                                <a href="?page=adm_achat_point&num=<?php echo ($page + 1); ?>&ticketParPage=<?php echo $achatParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-chevron-right"></i></a>
                                <a href="?page=adm_achat_point&num=<?php echo $moyenne; ?>&ticketParPage=<?php echo $achatParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-angle-double-right"></i></a>
                            <?php else : ?>
