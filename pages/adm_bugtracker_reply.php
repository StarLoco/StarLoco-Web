...
<div class="leftside">
    <ol class="breadcrumb">
        ...
        <li><a href="?page=adm_ticket">Bugtracker</a></li>
        <li class="active">Envoie bug</li>
    </ol>
    <?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $query = $web->prepare("INSERT INTO website_bugtracker_replies (ticket_id, reply_content, date, pseudo) VALUES (?, ?, ?, ?)");
        $query->execute([$ticketId, $replyContent, $replyDate, $replyPseudo]);

        // Mettez à jour le statut du ticket si nécessaire
        $query = $web->prepare("UPDATE website_bugtracker_site SET status = ? WHERE id = ?");
    ...
    <html>
    <head>
        <meta http-equiv="refresh" content="2;url=?page=adm_bugtracker_view&id=<?php echo $ticketId; ?>">
    </head>
    <body>
