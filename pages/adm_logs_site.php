...
<div class="leftside">
    <ol class="breadcrumb">
        ...
        <li class="active">Logs site</li>
    </ol>
    <div class="row">
        <div class="col-md-12 col-xs-12">
            <section class="no-border no-padding-top">
                <div class="page-header margin-top-10"><h4>Gestions des logs du site</h4></div>
            </section>
        </div>
    </div>
            ...
            <section class="section section-white no-border no-padding-top">
                <div class="section section-default padding-25">
                    ...
                    if(isset($_GET['wAchat']) && is_numeric($_GET['wAchat']) && isset($_GET['remove'])) {
                        $query = $web -> prepare("DELETE FROM `website_logs` WHERE `id` = ?;");
                        $query -> bindParam(1, $_GET['wAchat']);
                        $query -> execute();
                        $query -> closeCursor();
                        echo '<div class="alert alert-success no-border-radius no-margin" role="alert"><strong>Oh good!</strong> Le log a été supprimer avec succès !</div><br>';
                        echo '<script>setTimeout(function() {window.location.href = "?page=adm_logs_site";}, 1000); // 1000 millisecondes = 1 secondes</script>';
                    } ?>
                    <div class="box no-border-radius">
                        <table class="table table-striped no-margin">
                            ...
                            $queryCount = $web->prepare("SELECT COUNT(*) FROM `website_logs`;");
                            $queryCount->execute();
                            $nombreDeLog = $queryCount->fetchColumn(); // Le nombre total de comptes dans votre tableau
                            ...
                            $moyenne = ceil($nombreDeLog / $logParPage); // Nombre total de pages
                            ...
                            $indexDebut = max(($page - 1), 0) * $logParPage;

                            $query = $web -> prepare("SELECT * FROM `website_logs` ORDER BY `id` DESC LIMIT $indexDebut, $logParPage;");
                            $query -> execute();
                                ...
                                <tr>
                                    <td colspan="8">
                                        <div class='alert alert-info no-border-radius no-margin' style='text-align: center!important;' role='alert'>
                                            <strong>Oh non!</strong> Il n'y a aucune log!
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
                    ...
                    <center>
                        <div class="btn-group">
                            <a href="?page=adm_logs_site&num=1&logParPage=<?php echo $logParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-angle-double-left"></i></a>
                            <?php if ($page > 1) : ?>
                                <a href="?page=adm_logs_site&num=<?php echo ($page - 1); ?>&logParPage=<?php echo $logParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-chevron-left"></i></a>
                            <?php else : ?>
                                <a href="#" class="btn btn-sm btn-default"><i class="fa fa-chevron-left"></i></a>
                            <?php endif; ?>
                            <?php
                            // Afficher le numéro de page 1
                            echo '<a href="?page=adm_logs_site&num=1&logParPage=' . $logParPage . '" class="btn btn-sm ' . ($page == 1 ? 'btn-primary' : 'btn-default') . '">1</a>';
                            ...
                            for ($i = max(2, $page - 3); $i <= min($page + 3, $moyenne - 1); $i++) {
                                echo '<a href="?page=adm_logs_site&num=' . $i . '&logParPage=' . $logParPage . '" class="btn btn-sm ' . ($i == $page ? 'btn-primary' : 'btn-default') . '">' . $i . '</a>';
                            }
                            ...
                            echo '<a href="?page=adm_logs_site&num=' . $moyenne . '&logParPage=' . $logParPage . '" class="btn btn-sm ' . ($page == $moyenne ? 'btn-primary' : 'btn-default') . '">' . $moyenne . '</a>';
                            ?>
                            <?php if ($page < $moyenne) : ?>
                                <a href="?page=adm_logs_site&num=<?php echo ($page + 1); ?>&logParPage=<?php echo $logParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-chevron-right"></i></a>
                            <?php else : ?>
                                <a href="#" class="btn btn-sm btn-default"><i class="fa fa-chevron-right"></i></a>
                            <?php endif; ?>
                            <a href="?page=adm_logs_site&num=<?php echo $moyenne; ?>&logParPage=<?php echo $logParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-angle-double-right"></i></a>
                        </div>
                    </center>
                </div>
            </section>
