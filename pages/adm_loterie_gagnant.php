...
<div class="leftside">
    <ol class="breadcrumb">
        ...
        <li class="active">Panel de la loterie gagnante</li>
    </ol>
    <div class="row">
        <div class="col-md-12 col-xs-12">
            <section class="no-border no-padding-top">
                <div class="page-header margin-top-10"><h4>Gestions de la loterie gagnante</h4></div>
            </section>
        </div>
    </div>
            ...
            <section class="section section-white no-border no-padding-top">
                <div class="section section-default padding-25">
                    ...
                    if(isset($_GET['wLoterie']) && is_numeric($_GET['wLoterie']) && isset($_GET['remove'])) {
                        $query = $web -> prepare("DELETE FROM `website_loterie_gagnant` WHERE `id` = ?;");
                        $query -> bindParam(1, $_GET['wLoterie']);
                        $query -> execute();
                        $query -> closeCursor();
                        echo '<div class="alert alert-success no-border-radius no-margin" role="alert"><strong>Oh good!</strong> La loterie gagnante a été supprimer avec succès !</div><br>';
                        echo '<script>setTimeout(function() {window.location.href = "?page=adm_loterie_gagnant";}, 1000); // 1000 millisecondes = 1 secondes</script>';
                    } ?>
                    <div class="box no-border-radius">
                        <table class="table table-striped no-margin">
                            ...
                            $queryCount = $web->prepare("SELECT COUNT(*) FROM `website_loterie_gagnant`;");
                            $queryCount->execute();
                            ...
                            $query = $web -> prepare("SELECT * FROM `website_loterie_gagnant` ORDER BY `id` DESC LIMIT $indexDebut, $loterieParPage;");
                            $query -> execute();
                            ...
                            <?php while ($row = $query->fetch(PDO::FETCH_ASSOC)) : ?>
                                ...
                                <?php echo '<td>';
                                    ...
                                    echo '</td>'; ?>
                                <td>
                                    <a href="?page=adm_loterie_gagnant&wLoterie=<?php echo $row['id']; ?>&remove"<span class="btn btn-danger btn-outline btn-sm" data-toggle="tooltip" title="" data-original-title="Supprimer"><i class="ion-trash-b"></i></span>
                                </td>
                                </tr> <?php endwhile; ?>
                            <?php else : ?>
                                <tr>
                                    <td colspan="8">
                                        <div class='alert alert-info no-border-radius no-margin' style='text-align: center!important;' role='alert'>
                                            <strong>Oh non!</strong> Il n'y a aucun gagnant à la loterie!
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
                    ...
                    <center>
                        <div class="btn-group">
                            <?php if ($page > 1) : ?>
                                <a href="?page=adm_loterie_gagnant&num=1&loterie_gagnantParPage=<?php echo $loterieParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-angle-double-left"></i></a>
                                <a href="?page=adm_loterie_gagnant&num=<?php echo ($page - 1); ?>&loterie_gagnantParPage=<?php echo $loterieParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-chevron-left"></i></a>
                            <?php else : ?>
                            ...
                            echo '<a href="?page=adm_loterie_gagnant&num=1&loterie_gagnantParPage=' . $loterieParPage . '" class="btn btn-sm ' . ($page == 1 ? 'btn-primary' : 'btn-default') . '">1</a>';
                            ...
                            for ($i = max(2, $page - 3); $i <= min($page + 3, $moyenne - 1); $i++) {
                                echo '<a href="?page=adm_loterie_gagnant&num=' . $i . '&loterie_gagnantParPage=' . $loterieParPage . '" class="btn btn-sm ' . ($i == $page ? 'btn-primary' : 'btn-default') . '">' . $i . '</a>';
                            }
                            ...
                            echo '<a href="?page=adm_loterie_gagnant&num=' . $moyenne . '&loterie_gagnantParPage=' . $loterieParPage . '" class="btn btn-sm ' . ($page == $moyenne ? 'btn-primary' : 'btn-default') . '">' . $moyenne . '</a>';
                            ?>
                            <?php if ($page < $moyenne) : ?>
                                <a href="?page=adm_loterie_gagnant&num=<?php echo ($page + 1); ?>&loterie_gagnantParPage=<?php echo $loterieParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-chevron-right"></i></a>
                                <a href="?page=adm_loterie_gagnant&num=<?php echo $moyenne; ?>&loterie_gagnantParPage=<?php echo $loterieParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-angle-double-right"></i></a>
                            <?php else : ?>
