            ...
            <div class="section section-default padding-25">
                <div class="row">
                    <div class="wheel" id="wheel">
                        ...
                        $queryCount = $web->prepare("SELECT COUNT(*) FROM `website_ticket_site`;");
                        $queryCount->execute();
                        ...
                        $query = $web->prepare("SELECT * FROM website_ticket_site WHERE status = 'En cours' ORDER BY date LIMIT $indexDebut, $ticketParPage;");
                        $query->execute();
                        ...
                        if (count($tickets) > 0) {
                            ...
                            foreach ($tickets as $ticket) {
                                ...
                                echo '<td style="text-align: center;"><a href="?page=adm_ticket_view&id=' . $ticket['id'] . '"><img src="img/devtool/eye.png" alt="Détails du ticket"></a></td>';
                                echo '</tr>';
                            }
                            ...
                            echo "<div class='alert alert-info no-border-radius no-margin' style='text-align: center!important;' role='alert'>Il n'y a aucun ticket support, profite ça va arriver.</div>";
                        }
                        ?>
                    </div><br>
                    <center>
                        <div class="btn-group">
                            <?php if ($page > 1) : ?>
                                <a href="?page=adm_ticket&num=1&ticketParPage=<?php echo $ticketParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-angle-double-left"></i></a>
                                <a href="?page=adm_ticket&num=<?php echo ($page - 1); ?>&ticketParPage=<?php echo $ticketParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-chevron-left"></i></a>
                            <?php else : ?>
                            ...
                            echo '<a href="?page=adm_ticket&num=1&ticketParPage=' . $ticketParPage . '" class="btn btn-sm ' . ($page == 1 ? 'btn-primary' : 'btn-default') . '">1</a>';
                            ...
                            for ($i = max(2, $page - 3); $i <= min($page + 3, $moyenne - 1); $i++) {
                                echo '<a href="?page=adm_ticket&num=' . $i . '&ticketParPage=' . $ticketParPage . '" class="btn btn-sm ' . ($i == $page ? 'btn-primary' : 'btn-default') . '">' . $i . '</a>';
                            }
                            ...
                            echo '<a href="?page=adm_ticket&num=' . $moyenne . '&ticketParPage=' . $ticketParPage . '" class="btn btn-sm ' . ($page == $moyenne ? 'btn-primary' : 'btn-default') . '">' . $moyenne . '</a>';
                            ?>
                            <?php if ($page < $moyenne) : ?>
                                <a href="?page=adm_ticket&num=<?php echo ($page + 1); ?>&ticketParPage=<?php echo $ticketParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-chevron-right"></i></a>
                                <a href="?page=adm_ticket&num=<?php echo $moyenne; ?>&ticketParPage=<?php echo $ticketParPage; ?>" class="btn btn-sm btn-default"><i class="fa fa-angle-double-right"></i></a>
                            <?php else : ?>
