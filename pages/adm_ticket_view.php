...
<div class="leftside">
    <ol class="breadcrumb">
        ...
        <li class="active">Ticket en cours</li>
    </ol>
    ...
    <div class="wheel" id="wheel">
        ...
        $ticketId = $_GET['id'];
        $query = $web->prepare("SELECT * FROM website_ticket_site WHERE id = ?");
        ...
        $replyPseudo = $ticketDetails['account'];

        echo '<h2>Sujet : ' . $ticketDetails['subject'] . '</h2>';
        ...
        echo'<p>Account : '. $ticketDetails['account'].'</p>';
        echo'<p>Pseudo : '.$replyPseudo.'</p>';
        ...
        $queryReplies = $web->prepare("SELECT * FROM website_ticket_replies WHERE ticket_id = ?");
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                $updateQuery = $web->prepare("UPDATE website_ticket_site SET status = 'clôturé' WHERE id = :ticket_id");
                $updateQuery->bindParam(':ticket_id', $ticketId);
                $updateQuery->execute();

                header("Location:?page=adm_ticket_view&id=" . $ticketId);
                exit();
        ...
        ?>
            ...
            <form action="?page=adm_ticket_reply" method="post" onsubmit="return validateForm()">
                <input type="hidden" name="ticket_id" value="<?php echo $ticketId; ?>">
                <div class="form-group">
                    <textarea class="form-control" name="reply_content" id="editor" rows="4"></textarea>
                </div>
                <button type="submit" class="btn btn-primary">Répondre</button>
            </form><button id="clotureButton" class="btn btn-primary btn-red">Clôturer le ticket</button><br>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                ...
                clotureButton.addEventListener("click", function () {
                    ...
                    xhr.open("POST", "?page=adm_ticket_cloture", true);
                    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            // Redirige vers la page actuelle avec l'ID du ticket
                            window.location.href = "?page=adm_ticket";
                        }
                    };
    ...
    <div style="text-align: right;">
        <a href="?page=adm_ticket">Retour à la liste des tickets</a>
    </div>
</div>